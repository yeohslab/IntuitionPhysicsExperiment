    ...
    cfg.blocks.forEach(function (block, bi) {
      ...
      block.trials.forEach(function (t, ti) {
        function num(x, label) {
          var n = Number(x);
          if (!isFinite(n)) throw new Error('Block ' + (bi + 1) + ' 试次 ' + (ti + 1) + ': ' + label + ' 无效');
          return n;
        }
        ...
        if (L <= 0 || L > 2000) throw new Error('Block ' + (bi + 1) + ' 试次 ' + (ti + 1) + ': 摆长须在 (0,2000]');
        if (Number(t.visibleMs) < 0 || Number(t.invisibleMs) < 0) {
          throw new Error('Block ' + (bi + 1) + ' 试次 ' + (ti + 1) + ': 阶段时长不能为负');
        }
      });
    });
    return cfg;
  ...
  function buildTimeline(jsPsych, cfg) {
    ...
    timeline.push({
      ...
      stimulus:
        ...
        '<p>实验即将开始。</p>' +
        '<ul>' +
        '<li>每个试次有一条<strong>时间轴</strong>：前半为<strong>可视</strong>、后半为<strong>不可视</strong>（与配置一致）。可随时拖动时间轴查看该时刻的状态；不可视段内摆绳与摆球会隐藏并叠加浅灰遮罩。</li>' +
        '<li>熟悉后请点击<strong>汇报试次结束时刻的摆球位置</strong>，再拖动<strong>蓝色摆球</strong>汇报<strong>时间轴最右端（试次结束）</strong>时您认为摆球应在的角度。</li>' +
