...
EXPECTED_FORMAL_PER_SUBJECT = 125
EXPECTED_TRIALS_PER_E_LEVEL = 40
...
def run_qc_checks(formal: pd.DataFrame) -> list[str]:
    ...
    for sid, sub in formal.groupby("subject_id"):
        n = len(sub)
        status = "OK" if n == EXPECTED_FORMAL_PER_SUBJECT else "WARN"
        lines.append(f"- 被试 {sid}: {n} 试次 [{status}]")

    lines.append("")
    e_counts = formal.groupby("E_level").size()
    bad_e = e_counts[e_counts != EXPECTED_TRIALS_PER_E_LEVEL]
    if bad_e.empty:
        lines.append(f"- 25 能量水平各 {EXPECTED_TRIALS_PER_E_LEVEL} 试次: OK")
    else:
        lines.append("- 能量水平试次数异常:")
        for level, cnt in bad_e.items():
            lines.append(f"  - E={level}: {cnt}")
