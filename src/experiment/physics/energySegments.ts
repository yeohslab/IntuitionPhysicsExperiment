...
/** 组内所有正式 Block 的最高能量（各能量段中点的最大值）。 */
export function maxFormalBlockEnergyForGroup(group: MotionGroup): number {
  return Math.max(...buildKeptEnergySegmentsForGroup(group).map((segment) => segment.Emid));
}

/** 组级速度条上限：最高能量正式 Block 在最低点的绝对线速度。 */
export function speedBarVMaxForGroup(group: MotionGroup): number {
  const energyJ = maxFormalBlockEnergyForGroup(group);
  return ROD_LENGTH_M * pendulumOmegaRadForEnergyAtBottom(energyJ, ROD_LENGTH_M);
}
