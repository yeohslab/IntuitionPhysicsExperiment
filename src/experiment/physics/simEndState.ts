...
export function pendulumThetaAtSimEnd(
  ...
  maxRotationSubStep = 1 / 4000,
): number {
  return pendulumStateAtSimEnd(p, timing, maxRotationSubStep).theta;
}

/** 设计终止时刻（遮挡结束）的规范状态；旋转支默认使用 1/4000 s 子步。 */
export function pendulumStateAtSimEnd(
  ...
  if (analysis.regime === "rotation") {
    ...
    rot.step(simEndSec, maxRotationSubStep);
