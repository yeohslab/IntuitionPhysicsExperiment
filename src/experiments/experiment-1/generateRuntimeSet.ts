...
import { pendulumEnergy, pendulumPeriod, pendulumRegime, type PendulumParams } from "../../experiment/physics/pendulum";
...
import { HIDE_LEVELS_SEC, fadeTForGroup, showLevelsForGroup } from "../../experiment/physics/timePhases";
import {
...
export const TOTAL_RUNTIME_TRIALS =
  (NUM_FORMAL_BLOCKS + 1) * TRIALS_PER_FORMAL_BLOCK;

/** 组内 3×3 timing 全交叉：hide ∈ {0.8,1.0,1.2}s × 组专属 show 水平 */
...
export function generateRuntimeStimulusSet(opts: GenerateRuntimeSetOptions): ExperimentStimulusSet {
  ...
  const sequence: TopLevelSequenceItem[] = [
    ...
    makePracticeBlock(opts.group, rng, reportTrialGenerated),
  ];
