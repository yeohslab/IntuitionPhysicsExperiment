...
import type { Experiment3GenerateWorkerResponse } from "./generateWorker";

export type GenerateExperiment3RuntimeSetAsyncOptions = {
  subjectId: string;
  signal?: AbortSignal;
  onProgress?: (completedTrials: number, totalTrials: number) => void;
};

export function generateExperiment3RuntimeStimulusSetAsync(
  options: GenerateExperiment3RuntimeSetAsyncOptions,
): Promise<Experiment3StimulusSet> {
  return new Promise((resolve, reject) => {
    ...
    worker.addEventListener(
      "message",
      (event: MessageEvent<Experiment3GenerateWorkerResponse>) => {
    ...
    worker.addEventListener("error", (event) => {
      finish(() => reject(new Error(event.message || "实验三刺激生成线程异常退出")));
    });
